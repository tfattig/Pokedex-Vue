<template>
	...
	<div class="detail">
		<!-- Display spinner while data is loading, else show the Pokemon details -->
		<div class="detail-view card" v-if="show">
			...
			<!-- Display Pokemon base experience, height, and weight -->
			<div class="property">
					<div class="left highlight">Base Experience</div>
					<div class="right">{{ pokemon.base_experience }} XP</div>
			</div>
			<div class="property">
					<div class="left highlight">Height</div>
					<div class="right">{{ pokemon.height / 10 }} m</div>
			</div>
			<div class="property">
					<div class="left highlight">Weight</div>
			...
			<div class="abilities">
				...
				<!-- Display additional inactive abilities in a compact form -->
				<div class="right inactive">{{pokemon.moves.map(item => ' ' + item.move.name).toString()}} .</div>
				</div>
			</div>
			<!-- Display a message if the Pokemon was not found -->
...
<script>
	export default {
		...
		data: () => {
			return {
				show: false,    // Flag to show/hide Pokemon details
		...
		methods: {
			...
			// Emit event to parent component to close the detail view
			closeDetail() {
					this.$emit('closeDetail');
			}
		},
			created() {
				// Fetch Pokemon data on component creation
				this.fetchData();
			}
	}
</script>
...
/* Styling for spinner icon */
i.fa-spinner {
	text-align: center;
}

/* Styling for the Pokemon detail container */
.detail {
	display: flex;
...
/* Styling for the Pokemon detail view */
.detail-view {
	display: flex;
	justify-content: center;
	align-items: center;
	...
	border-radius: 5px;
	/* Box shadow for a subtle visual effect */
	box-shadow: 0 15px 30px rgba(0,0,0,.2), 0 10px 10px rgba(0,0,0,.2);
}

/* Styling for Pokemon image */
.image {
	display: flex;
	justify-content: center;
	align-items: center;
...
/* Styling for Pokemon data container */
.data {
	display: flex;
